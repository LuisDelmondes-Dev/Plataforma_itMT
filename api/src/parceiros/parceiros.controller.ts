...
export class ParceirosController {
  ...
  async submeter(@Body() dto: ContribuicaoDto, @Req() req: TenantRequest & { usuario: Sessao }) {
    ...
    const r = await this.db.withTenantTransaction(req.tenantContext!, () => this.db.query<{ id: number }>(
  ...
  async pendentes(@Req() req: TenantRequest) {
    const r = await this.db.withTenantTransaction(req.tenantContext!, () => this.db.query(
  ...
  async parecer(
    ...
    const r = await this.db.withTenantTransaction(req.tenantContext!, () => this.db.query<{ titulo: string }>(
  ...
  @Papeis('PARCEIRO', 'UNIVERSIDADE', 'CURADOR', 'ADMIN')
  async minhas(@Req() req: TenantRequest & { usuario: Sessao }) {
    const r = await this.db.withTenantTransaction(req.tenantContext!, () => this.db.query(
      `SELECT "ContribuicaoDado_Id" AS id, "ContribuicaoDado_Quando"::text AS quando,
              "ContribuicaoDado_Tipo" AS tipo, "ContribuicaoDado_Titulo" AS titulo,
